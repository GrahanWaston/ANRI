        ...
        <div class="page-header d-print-none">
            <div class="row align-items-center">
                ...
                <div class="col-auto d-print-none">
                    <div class="btn-list">
                        <a href="/publikasi-file/create" class="btn btn-primary d-inline-block">
                            <i class="fa fa-plus me-2"></i> Publikasi File
                        </a>
                    </div>
                </div>
            </div>
                        ...
                        <tbody>
                            @foreach ($file as $files)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <a href="/publikasi-file/{{ $files->id }}/edit"
                                                class="btn btn-light m-1 h3 text-primary"><i
                                                    class="fas fa-file-signature"></i></a>
                                            @if (auth()->user()->role == 'admin')
                                                @if ($files->status == 'draft' || $files->status == '')
                                                    <form action="/approve-file/{{ $files->id }}" method="POST"
                                                        class="d-inline">
                                                        @method('PUT')
                                                        @csrf
                                                ...
                                                @else
                                                    <form action="/unapprove-file/{{ $files->id }}" method="POST"
                                                        class="d-inline">
                                                        @method('PUT')
                                                        @csrf
                                                ...
                                                @endif
                                            @endif
                                            <form action="{{ route('publikasi-file.destroy', $files->id) }}" method="POST"
                                                class="d-inline">
                                                @method('delete')
                                                @csrf
                                                <button type="submit" class="btn btn-light m-1 h3 text-danger">
    ...
    <script type="text/javascript">
        $(function() {
            $('#navbar-menu > .navbar-nav > .nav-item > .nav-link > .nav-link-title:contains("Publikasi")').parents(
                ...
                '.dropdown-item:contains("File Download")').addClass('active');
        });
    </script>
